...
do forever
	...
	select
		...
		when var=="extra" | var=="what" | var=="sdum" then do
			var = "extra"
			e = e + 1
		...
		when var=="group" | var=="type" | var=="disable" then
			call writeval fout, var, value

		otherwise
			/* all other variables */
			call lineout "<STDERR>"
			call lineout "<STDERR>", "ERROR Unknown variable in card" card
			call lineout "<STDERR>", "ERROR" line
			call lineout "<STDERR>", "ERROR" var "=" value
			exit
	end
end
